...
__global__ void softmax_forward_kernel5(float* out, float inv_temperature, const float* inp, int N, int T) {
    ...
    for (int i = warp.thread_rank(); i <= own_pos; i += warp.size()) {
        ...
        //if (idx==0) printf("idx: %u, i: %u, ev: %f, norm: %f\n", idx, i, ev, norm);
...
void gpt2_forward(GPT2 *model, int* inputs, int* targets, int B, int T, int genIndex) {
        ...
        */
        ...
        if (step == 0 && step % sample_every == 0 || last_step) {
            ...
            for (int t = 1; t < genT; t++) {
                ...
                gpt2_forward(&model, gen_tokens, NULL, B, T, t-1);
        ...
        gpt2_forward(&model, train_loader.inputs, train_loader.targets, B, T, 0);
        gpt2_zero_grad(&model);
        gpt2_backward(&model);
