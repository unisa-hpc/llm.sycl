            ...
            auto event = q.submit([&](sycl::handler &h) {
                ...
                h.parallel_for(
                        ...
                        [=](sycl::nd_item<1> item) {
                            ...
                            const size_t subgroup_size = item.get_sub_group().get_local_range()[0]; // Get the size of the subgroup
                            ...
                            for (auto i = idInSg; i < pos_by_4; i += subgroup_size) {
                                float val0 = accTnInp[idx * capturedT + i + 0];
                                float val1 = accTnInp[idx * capturedT + i + 1];
                                float val2 = accTnInp[idx * capturedT + i + 2];
                                float val3 = accTnInp[idx * capturedT + i + 3];
                            ...
                            if(4*pos_by_4 + idInSg <= own_pos) {
                                float old_maxval = maxval;
                                auto vv = accTnInp[idx * capturedT + 4*pos_by_4 + idInSg];
