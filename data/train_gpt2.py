...
if __name__ == "__main__":
    ...
    parser.add_argument("--input_bin", type=str, default="dataset_prepared/tiny_shakespeare_val.bin",
    ...
    if master_process and args.write_tensors:  # tokenizer is technically not tensors but ok
        write_tokenizer(enc, "dataset_prepared/gpt2_tokenizer.bin")
    ...
    if master_process and (not args.inference_only and args.write_tensors):
        ...
        write_model(model, f"dataset_prepared/gpt2_{model_size_str}.bin", dtype="float32")
        write_model(model, f"dataset_prepared/gpt2_{model_size_str}_bf16.bin", dtype="bfloat16")
        # save x, y, logits, loss, and parameter gradients, for debugging C
        # always store these in fp32 to have an accurate reference (?)
        write_state(model, x, y, logits, loss, f"dataset_prepared/gpt2_{model_size_str}_debug_state.bin")
    ...
    for i in range(args.num_iterations):
        ...
        print0(
            f"iteration {i + 1}, loss: {loss.item():.4f}, time: {(t1 - t0) * 1000:.3f}ms, tok/s: {tokens_per_second:.2f}")
